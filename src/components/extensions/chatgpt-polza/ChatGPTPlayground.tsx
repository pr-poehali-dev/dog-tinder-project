        ...
        <div className="flex-1 overflow-y-auto p-6">
          {messages.length === 0 ? (
            <div className="h-full flex items-center justify-center">
              <div className="text-center text-gray-500 max-w-2xl mx-auto px-4">
                <svg className="w-12 h-12 mx-auto mb-3 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                </svg>
                <p className="mb-6">Ваш диалог появится здесь</p>
                
                {quickQuestions.length > 0 && (
                  <div className="mt-6">
                    <p className="text-sm text-gray-400 mb-3">Частые вопросы:</p>
                    <div className="flex flex-wrap gap-2 justify-center">
                      {quickQuestions.map((question, idx) => (
                        <button
                          key={idx}
                          onClick={() => setInput(question)}
                          className="px-4 py-2 bg-[#2d2d2d] hover:bg-[#3d3d3d] rounded-lg text-sm text-left text-white transition-colors border border-gray-600 hover:border-gray-500"
                        >
                          {question}
                        </button>
                      ))}
